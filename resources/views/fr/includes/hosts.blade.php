<div class="flex flex-col" id="hosts">

    <h3 class="text-5xl mt-10 mb-5">Hosts</h3>

    <div class="lg:flex">

        <figure class="md:flex md:justify-end lg:w-3/12">

            <img class="md:w-8/12 lg:w-full" src="{{ asset('img/bert-van-steenberghe.png') }}" alt="Foto van Bert van Steenberghe" />

        </figure>

        <div class="lg:w-8/12 mt-8 lg:-mt-10 lg:ml-10">

            <h3 class="text-2xl text-4xl lg:mt-16 mb-5 text-left">Bert van Steenberghe</h3>

            <p class="lg:text-lg text-left">Bert van Steenberghe est un présentateur radio chez Studio Brussel. Il utilise également ses connaissances et sa passion pour la musique en tant que DJ. Il prête régulièrement sa voix aux spots et aux annonces de la VRT. Bert n'est pas seulement un présentateur, mais plutôt un créateur de radio. </p>

        </div>

    </div>

    <div class="lg:flex flex-row-reverse mt-10">

        <figure class="md:flex md:justify-end lg:w-3/12">

            <img class="md:w-8/12 lg:w-full" src="{{ asset('img/aurore-picavet.png') }}" alt="Foto van Aurore Picavet" />

        </figure>

        <div class="lg:w-8/12 mt-8 lg:-mt-10 mr-10">

            <h3 class="text-2xl lg:text-4xl lg:mt-16 mb-5 lg:text-right">Aurore Schottey Picavet</h3>

            <p class="lg:text-lg lg:text-right">Aurore Schottey Picavet est animatrice radio et comédienne pour le théâtre, le cinéma et la télévision. Elle est également active en tant que voix off. Elle parle cinq langues : français, néerlandais, allemand, anglais et espagnol. Elle est mère, joue du violon et se soucie de la durabilité.</p>

        </div>

    </div>

</div>
